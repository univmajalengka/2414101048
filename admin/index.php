...
<head>
  ...
  <title>Daftar Produk | Jossmen Laundry</title>
        ...
        <div class="max-w-6xl mx-auto px-4 py-6 space-y-6">
          <section class="bg-white border border-gray-200 rounded-2xl shadow-sm p-6">
            <div class="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
              <div>
                <h2 class="text-lg font-semibold">Produk Terdaftar</h2>
                <p class="text-sm text-slate-500">
              ...
              <div class="flex items-center gap-3 text-sm">
                <input
                  ...
                  class="w-full md:w-60 px-4 py-2.5 rounded-xl border border-gray-200 focus:border-pink-500 focus:ring-2 focus:ring-pink-100 outline-none transition" />
              </div>
            </div>
              ...
              <table class="min-w-full text-sm">
                <thead>
                  <tr class="text-left text-slate-500 border-b border-gray-200">
                    ...
                    <th class="pb-3 pr-6">Deskripsi Singkat</th>
                    <th class="pb-3 pr-6">Harga</th>
                    <th class="pb-3 pr-6">Gambar</th>
                  ...
                  <?php if ($result->num_rows > 0): ?>
                    <?php while ($row = $result->fetch_assoc()): ?>
                      <tr class="hover:bg-pink-50/40">
                        ...
                        <td class="py-4 pr-6">
                          <?= nl2br(htmlspecialchars($row['subtitle'] ?? '-', ENT_QUOTES, 'UTF-8')) ?>
                        </td>
                        <td class="py-4 pr-6">
                          <?= htmlspecialchars($row['harga'] ?? '-', ENT_QUOTES, 'UTF-8') ?>
                        </td>
                        <td class="py-4 pr-6">
                          <?php
                          $imagePath = '../assets/images/' . ($row['image_path'] ?? '');
                          $fullPath = dirname(__DIR__) . '/assets/images/' . ($row['image_path'] ?? '');
                          ?>
                          <?php if (! empty($row['image_path']) && file_exists($fullPath)): ?>
                            <img
                              src="<?= htmlspecialchars($imagePath, ENT_QUOTES, 'UTF-8') ?>"
                              alt="<?= htmlspecialchars($row['title'] ?? 'Gambar produk', ENT_QUOTES, 'UTF-8') ?>"
                        ...
                        <td class="py-4">
                          <div class="flex items-center gap-2">
                            <a href="editProduk.php?id=<?= $row['id'] ?>"
                              class="px-4 py-2 rounded-xl border border-gray-200 hover:border-pink-500">
                              Edit
                            </a>
                            <a href="../services/deleteProdukService.php?id=<?= $row['id'] ?>"
                              class="px-4 py-2 rounded-xl bg-red-500 text-white hover:opacity-90"
